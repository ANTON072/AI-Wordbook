...
**[HUMAN] タスクの扱い**：AWS設定・デプロイ実行・シークレット入力など、人間が手動で行う必要があるタスクには先頭に `[HUMAN]` を付ける。Claude はそのタスクに到達したら作業を停止し、ユーザーに操作内容を伝えて完了報告を待つ。完了報告を受けたらそのタスクをチェック済みにして次へ進む。セッションを再開する場合は `tasklist.md` を読んで未完了の最初のタスクから再開する。
...
2. **コードレビューループ**：`code-reviewer` エージェントを起動し、完了通知を受け取ってから指摘一覧をユーザーに提示する。`code-reviewer` はサブエージェント（バックグラウンド）として実行されるため、完了通知が届くまで品質チェックへ進んではならない。重大度高（セキュリティ・層境界違反）は自動修正し、判断が分かれる指摘はユーザーに確認してから対応する。残存指摘がなくなるまで繰り返し、対応困難な指摘はその理由をユーザーに報告する。
3. **品質チェック**：コードレビューループの全指摘対応が完了した後に実行する。`pnpm check`（lint・format・test・typecheck を一括実行するコマンド）が all green になるまで、エラーを自動修正して繰り返す。all green でない限り作業は完了扱いにしない。
...
レビュー完了後、指摘を **1件ずつ** 以下の形式でユーザーに提示し、判断を受けてからドラフトを更新して次の指摘へ進む。まとめて提示・まとめて判断は行わない。

**各指摘の提示形式：**
1. 指摘内容（何が問題か）を1〜2文で説明する
2. 推奨対応を具体的に示す（例：「〇〇を△△に変更します」）
3. ユーザーの判断を待つ（「採用する」「別案にする」「スキップする」など）
4. 回答を受けてからファイルを更新し、「更新しました」と一言添えて次の指摘へ進む

指摘がゼロの場合は「指摘なし。ドラフト確定です」と伝えて終了する。
