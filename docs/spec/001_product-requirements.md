...
- **会話で登録**：Claude Desktop との会話の中で英単語の辞書情報を生成し、MCP ツールで単語帳に保存する
...
Claude Desktop から以下の操作を自然言語で実行できる。

| ツール名 | 操作 | 正常系の戻り値 | 異常系の戻り値 |
| --- | --- | --- | --- |
| `register_word` | 英単語・辞書情報（品詞・和訳・例文）を受け取り保存 | 単語の個別ページ URL | バリデーションエラーの内容 |
| `delete_word` | 単語を削除 | 削除完了メッセージ | 単語が存在しない旨のメッセージ |
| `update_word` | 単語の辞書情報（entries）を全置換で上書き保存（登録日時は保持） | 単語の個別ページ URL | 単語が存在しない旨のメッセージ |
...
- `update_word`：既存単語の辞書情報（entries）を全置換。登録日時（createdAt）は保持。Claude が辞書情報を再生成して渡す。単語が存在しない場合はエラー。
...
**複数品詞の扱い：** 1 単語 = 1 エントリ。`book` のように複数品詞がある場合、Claude が名詞・動詞などすべての品詞エントリを一括生成して配列として渡す。後から品詞エントリを追加する操作は非対応（更新が必要なら `update_word` で全再生成）。
...
**個別ページの URL キー：** 正規化済み単語を URL エンコードした値（URL エンコードの詳細例は[機能設計書（002）](./002_functional-design.md)を参照）。

#### 認証

- **Web：** Next.js が Cognito Hosted UI 経由でログイン。セッション失効時はログインページへリダイレクト（実装詳細は[技術仕様書（003）](./003_architecture.md)を参照）。
- **MCP：** OAuth 2.0 PKCE（Cognito 連携）。Claude Desktop がアクセストークンの取得・管理を担う（認証フローの詳細は[機能設計書（002）](./002_functional-design.md)・[技術仕様書（003）](./003_architecture.md)を参照）。
